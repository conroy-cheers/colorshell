...
export class Wallpaper extends GObject.Object {
    ...
    constructor() {
        ...
        try {
            this.#hyprpaperFile.get_parent()?.make_directory_with_parents(null);
        } catch(_) {}
        ...
        if(this.#hyprpaperFile.query_exists(null)) {
            try {
                ...
                console.warn("Wallpaper: Couldn't get wallpaper from hyprpaper config, falling back to cached data");
            }
        }

        if(!this.#wallpaper && this.#walFile.query_exists(null)) {
            try {
                this.#wallpaper = this.getData().wallpaper;
            } catch(_) {
                console.warn("Wallpaper: Couldn't get wallpaper from wal cache");
            }
        }
    ...
    private writeChanges(): void {
        const wallpaperBlock = this.#wallpaper?.trim() ? `
