...
export const PageBluetooth = () => {
    ...
    return <Page id={"bluetooth"} title={tr("control_center.pages.bluetooth.title")}
      spacing={6} description={tr("control_center.pages.bluetooth.description")}
      headerButtons={createBinding(Bluetooth.getDefault(), "adapter")(adapter => adapter ? [{
          iconName: createBinding(adapter, "discovering")
                    ...
                    <Gtk.Box orientation={Gtk.Orientation.VERTICAL} spacing={2}>
                        <For each={adapters}> 
                            {(adapter: AstalBluetooth.Adapter) => {
                                ...
                                return <Page.DecoratedButton class={isSelected(is => is ? "selected" : "")} 
...
function DeviceWidget({ device }: { device: AstalBluetooth.Device }): Gtk.Widget {
    const statusWidget = <Gtk.Box spacing={6}>
        ...
        <Gtk.Box visible={createComputed(() => {
            const batt = createBinding(device, "batteryPercentage")(),
                connected = createBinding(device, "connected")();

            return connected && batt > -1;
        })} spacing={4}>
    ...
    return <Page.DecoratedButton class={createBinding(device, "connected")(conn => 
      conn ? "selected" : "")} title={
        ...
        extraButtons={<With value={createComputed(() => {
              const connected =createBinding(device, "connected")(),
                  trusted = createBinding(device, "trusted")();
